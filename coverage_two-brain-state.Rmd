...
coverage_table <- coverage_table %>%
  mutate(cluster = factor(cluster),
         group = factor(case_when(
           nchar(as.character(id)) == 4 ~ group,
           nchar(as.character(id)) == 7 & substr(as.character(id), 5, 5) == 3 ~ "Intergen",
           nchar(as.character(id)) == 7 & substr(as.character(id), 5, 5) < 3 ~ "Same gen",
           TRUE ~ "None" 
         )),
...
ggsave("two-brain-state_byclusters.png", width = 7, height = 7, dpi = 400)
...
ggsave("two-brain-state_byclusters_wo.png", width = 18, height = 18  , dpi = 400)
```
