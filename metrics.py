# Calculating metrics of brain state sequence time series
...
# ------------------------------------------------------------
# Load Data
datapath = Path(path) / "2_clustering" 
outpath = Path(path) / "3_time-series-metrics"
...
# ------------------------------------------------------------
# Calculate coverage
metric_table, metric_table_single, entropy_table, entropy_table_single = [], [], [], []
sorted_tasks = sorted(set(results_table.tasks))
if "one-brain" in type_of_data: 
    true_dyads = pd.read_csv(Path(path) / "1_fNIRS_prepared" / "additional_information" / "dyadList.csv")
...
if "one-brain" in type_of_data: 
    metric_table_single = pd.DataFrame(metric_table_single,
...
if "one-brain" in type_of_data: 
    metric_table.to_csv((outpath / f"metrics-table_{type_of_data}_combined.csv"), index=False)
    metric_table_single.to_csv((outpath / f"metrics-table_{type_of_data}_single.csv"), index=False)
    ...
    metric_table.to_csv((outpath / f"metrics-table_{type_of_data}.csv"), index=False)
    entropy_table.to_csv((outpath / f"metrics-table_{type_of_data}.csv"), index=False)
